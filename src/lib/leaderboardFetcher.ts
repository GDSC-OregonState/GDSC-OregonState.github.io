const CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTBVgYpnpaDoLK_g6klbR0LYV-KTM8uvU7BSulbqHhZXt7Cg2BbauOIfYVibnlx_neTuNDgljO_db77/pub?output=csv';
...
export async function getLeaderboard() {
    ...
    const [headerRow, totalRow] = lines.slice(0, 2).map((line) => line.split(','));

    let liveParticipants: Participant[] = headerRow.slice(1)
        .filter((name) => name)
        .map((name, index) => ({
            name: name.trim(),
